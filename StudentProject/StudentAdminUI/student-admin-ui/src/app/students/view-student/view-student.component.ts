...
export class ViewStudentComponent implements OnInit {
  ...
  onDelete() {
    this.studentService.deleteStudent(this.student.id).subscribe(
      (success) => {
        this.snackbar.open('Öğrenci başarılı bil şekilde silindi', undefined, {
          duration: 2000,
        });
        setTimeout(() => {
          this.router.navigateByUrl('students');
  ...
  onAdd() {
    return this.studentService.addStudent(this.student).subscribe(
      (success) => {
      ...
      (err) => {
        this.snackbar.open('Öğrenci eklerken hata oluştu!', undefined, {
          duration: 2000,
        });
        setTimeout(() => {
          this.router.navigateByUrl(`students`);
